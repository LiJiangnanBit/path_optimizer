...
bool PathOptimizer::smoothPath(tk::spline *x_s_out, tk::spline *y_s_out, double *max_s) {
    return smoothPathFrenet(x_s_out, y_s_out, max_s);
//    return smoothPathCartesian(x_s_out, y_s_out, max_s);
}

bool PathOptimizer::smoothPathFrenet(tk::spline *x_s_out, tk::spline *y_s_out, double *max_s_out) {
...
bool PathOptimizer::smoothPathCartesian(tk::spline *x_s_out, tk::spline *y_s_out, double *max_s_out) {
    auto sm_start = std::clock();
    std::vector<double> x_list, y_list, s_list, angle_list;
    tk::spline x_spline, y_spline;
    double s = 0;
